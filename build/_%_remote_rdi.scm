...
(define (rdi-set-host! address) (set! default-remote-debugger-address address))
(define *rdi-function* (lambda (x) x))
(define (rdi-set-rdi-function! f) (set! *rdi-function* f))
  ...
  (call-with-input-string
   ...
   (lambda (port)
     (let* ((x (read-all port (lambda (port) (read-line port #\:))))
            (len (length x)))
       (cond ((<= len 1)
              (cons (if (= len 1) (car x) default-remote-debugger-address)
                    default-remote-debugger-port-num))
             ((= len 2)
              (let ((address (car x)) (port-num (string->number (cadr x) 10)))
                (if (and port-num
                         (exact? port-num)
                         (integer? port-num)
                         (>= port-num 1)
                    ...
                    #f)))
             (else #f))))))
(define-type rdi address port-num seq-num call-table connection writer-thread)
  ...
  (and remote-debugger-address
       (let ((x (split-address remote-debugger-address)))
         (if (not x)
             ...
             (let* ((address (car x))
                    (port-num (cdr x))
                    (rdi (make-rdi address port-num 0 (make-table) #f #f))
                    (writer-thread (rdi-create-writer-thread rdi)))
...
(define (rdi-create-server remote-debugger-port-num)
  (let* ((address #f)
         (port-num
          (or remote-debugger-port-num default-remote-debugger-port-num))
         (rdi (make-rdi address port-num 0 (make-table) #f #f))
         (writer-thread (rdi-create-writer-thread rdi)))
    (rdi-writer-thread-set! rdi writer-thread)
    (thread-start! writer-thread)
...
(define (rdi-force-connection rdi)
  (or (rdi-connection rdi)
      (if (rdi-address rdi) (rdi-open-client rdi) (rdi-open-server rdi))))
(define rdi-version1 '())
(define rdi-version2 '())
...
(define (rdi-open-server rdi)
  (let ((listen-port
         (open-tcp-server
          (list server-address:
                (string-append "*:" (number->string (rdi-port-num rdi)))
                reuse-address:
                #t))))
    (let loop ()
      (let ((connection (read listen-port)))
        (let ((request (read connection)))
          (if (not (equal? request rdi-version1))
              ...
              (begin
                ...
                (force-output connection)
                (let ((reader-thread
                       (rdi-create-reader-thread rdi connection)))
                  (rdi-connection-set! rdi connection)
                  (thread-start! reader-thread)
                  (loop)))))))))
     ...
     (let loop ()
       (let ((msg (read connection)))
         (if (not (eof-object? msg))
             (begin (thread-send (rdi-writer-thread rdi) msg) (loop)))))
   ...
   (lambda ()
     (let loop ()
       (let ((msg (thread-receive)))
         (and (rdi-handle-message rdi msg) (loop)))))))
...
(define (rdi-handle-message rdi msg)
  (if (pair? msg)
      (case (car msg)
        ((reader-thread-terminated) #t)
        ((terminate) #f)
        ((call)
         (let* ((seq-num (cadr msg))
                (call (caddr msg))
                (result (apply (*rdi-function* (car call)) (cdr call))))
        ...
        ((remote-call)
         (let* ((result-mutex (cadr msg))
           ...
           (table-set! (rdi-call-table rdi) seq-num result-mutex)
           #t))
        ((return)
         (let* ((seq-num (cadr msg))
                (result (caddr msg))
           ...
           (if (not result-mutex)
               ...
               (begin
                 ...
                 #t))))
        (else (pretty-print (list 'unhandled-message msg) ##stdout-port) #t))
      #f))

...
(define rdi #f)
(define (make-rdi-host host) (set! rdi (rdi-create-client host)))
(define (debuggee-rdi-function fn)
  (case fn
    ((console-input) rdi-console-input)
    (else (error "unknown function"))))
(define rdi-console-table (make-table))
(define (rdi-console-input console-id input)
  (let ((remote-port (table-ref rdi-console-table console-id #f)))
    (if remote-port
        (begin (display input remote-port) (force-output remote-port))))
  #t)
  ...
  (if (< start end)
      (begin
        ...
        (let ((n (read-substring str start (+ start 1) port)))
          (input-port-timeout-set! port -inf.0)
          (if (= n 1) (+ 1 (read-substring str (+ start 1) end port)) n)))
      0))
(define (rdi-console-output-pump-start! console-id remote-port)
  (thread-start!
   (make-thread
    (lambda ()
      (let* ((buflen 1000) (buf (make-string buflen)))
        (let loop ()
...
(define (make-repl-channel-remote-port thread)
  (receive (local-port remote-port)
           (open-string-pipe)
           (begin
...
(define (thread-make-repl-channel-remote thread)
  (with-exception-catcher
   (lambda (e)
     (let ((i (open-input-string "")) (o (open-output-string)))
